...
const Hero = () => {
    ...
    return (
        <div className='flex flex-col items-center justify-center mt-[110px] select-none'>
            ...
            <motion.div ref={t1} initial={{ opacity: 0 }} animate={elementVisible1 ? { opacity: 1 } : { opacity: 0 }} transition={{ delay: 0.1 }} className="text-center bg-gradient-to-b from-indigo-50 to-indigo-300 inline-block text-transparent bg-clip-text text-4xl sm:text-6xl max-w-sm sm:max-w-2xl font-bold leading-[50px] sm:leading-[70px] mb-4 ">An Opensource Cross Cloud Migration Tool</motion.div>
            <motion.div ref={t1} initial={{ opacity: 0 }} animate={elementVisible1 ? { opacity: 1 } : { opacity: 0 }} transition={{ delay: 0.1 }} className=" text-center text-indigo-300 text-lg font-normal leading-[31px] px-[20px] lg:px-[160px] xl:px-[420px] ">Xmigrate(pronounced as cross-migrate) is an opensource tool for migrating your VM'sfrom anywhere to cloud and cloud to anywhere seamlessly.</motion.div>
            <div className="justify-center items-start gap-6 inline-flex mt-[32px]">
                <motion.a href='#contact' ref={t1} initial={{ opacity: 0, x: -50 }} animate={elementVisible1 ? { opacity: 1, x: 0 } : { opacity: 0, x: 0 }} transition={{ delay: 0.5 }} className=" pl-[22.23px] pr-[21.77px] pt-[16.22px] pb-[16.78px] bg-slate-800 hover:bg-slate-700 rounded-[58px] justify-center items-center flex ">
                    <div className="w-[103px] h-5 text-center text-violet-300 hover:text-violet-200 text-base font-semibold leading-snug">Start Migrate</div>
                </motion.a>
                <motion.a href='https://www.youtube.com/watch?v=FISJaoUIiOM' target="_blank" ref={t1} initial={{ opacity: 0, x: 50 }} animate={elementVisible1 ? { opacity: 1, x: 0 } : { opacity: 0, x: 0 }} transition={{ delay: 0.5 }} className=" pl-[25px] pr-[24.66px] pt-4 pb-[17.39px] bg-indigo-900 hover:bg-indigo-900 bg-opacity-20 rounded-[58px] border border-slate-800 justify-center items-center flex">
                    <div className="w-[90px] h-5 text-center text-violet-300 hover:text-violet-200 text-base font-semibold leading-snug">View Demo</div>
                </motion.a>
            </div>
            <img
                loading="lazy"
            ...
            <motion.div ref={t3} initial={{ opacity: 0 }} animate={elementVisible3 ? { opacity: 1 } : { opacity: 0 }} transition={{ duration: 0.8 }} id='features' className="text-center mt-12 pt-12 pb-18 bg-gradient-to-b from-indigo-50 to-indigo-300 inline-block text-transparent bg-clip-text text-4xl sm:text-5xl font-bold leading-[50px] sm:leading-[60px] max-w-[350px] sm:max-w-4xl">Don't settle on <br className='sm:hidden block' />one cloud,<br /> have a hybrid strategy for recovery</motion.div>
